...
class Canvas:
    ...
    def render_hands(self, event):
        ...
        for i in range(0, len(event.hands)):
            # Hand is a class that contains the hand data, left or right
            hand = event.hands[i]
            ...
            for index_digit, digit_name in enumerate(
                ["Thumb", "Index", "Middle", "Ring", "Pinky"]
            ):
                # Digit is a class that contains the finger data, in sequence contains thumb, index, middle, ring, pinky
                digit = hand.digits[index_digit]
                print(f"  {digit_name} Finger")
                # for index_bone in range(0, 4):
                for index_bone, bone_name in enumerate(
                    ["Metacarpal", "Proximal", "Intermediate", "Distal"]
                ):
                    # Bone is a class that contains the bone data, in sequence, metacarpal, proximal, intermediate, distal
                    ...
                    if self.hands_format == "Skeleton":
                        # Wrist and Elbow position comes from the arm class
