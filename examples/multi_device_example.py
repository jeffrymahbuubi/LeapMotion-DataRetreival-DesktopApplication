...
class TrackingEventListener(leap.Listener):
    ...
    def number_of_devices_tracking(self):
        device_latest_tracking_event = self.device_latest_tracking_event
        print(f"Number of devices tracking: {len(device_latest_tracking_event)}")
        return len(device_latest_tracking_event)
    ...
    def on_tracking_event(self, event):
        if event.tracking_frame_id % 2 == 0:
            for hand in event.hands:
                ...
                print(
                    f"Frame {event.tracking_frame_id} with {hand_type} hands on device {event.metadata.device_id}"
                )
                # print(
...
def main():
    ...
    with connection.open():
        wait_until(lambda: device_listener.n_events == 3)
