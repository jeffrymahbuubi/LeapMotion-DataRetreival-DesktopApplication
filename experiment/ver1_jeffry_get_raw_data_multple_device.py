            ...
            for hand in event.hands:
                ...
                for index_digit, digit in enumerate(hand.digits):
                    for index_bone, bone in enumerate(digit.bones):
                        key = (
                            timestamp,
                            self.finger_names[index_digit],
                            self.bone_names[index_bone],
                        )
                        ...
                        self.bone_coordinates[key].update(
                            {
                                f"start_x_{device_identifier}": bone.prev_joint.x,
                                f"start_y_{device_identifier}": bone.prev_joint.y,
                                f"start_z_{device_identifier}": bone.prev_joint.z,
                                f"end_x_{device_identifier}": bone.next_joint.x,
                                f"end_y_{device_identifier}": bone.next_joint.y,
                                f"end_z_{device_identifier}": bone.next_joint.z,
                            }
                        )
